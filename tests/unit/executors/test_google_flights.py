...
from flight_finder.executors.google_flights import parse_results_html
from flight_finder.models.result import SiteResult

_FIXTURE = Path(__file__).parents[2] / "fixtures" / "mock_pages" / "google_flights_search.html"
...
class TestParseResultsHtml:
    ...
    def test_all_are_site_results(self) -> None:
        results = parse_results_html(_load_fixture(), "SFO", "CDG", _CAPTURED_AT)
        for r in results:
            assert isinstance(r, SiteResult)
            assert r.adapter == "google_flights"
            assert r.captured_at == _CAPTURED_AT

    def test_nonstop_flight_parsed(self) -> None:
        results = parse_results_html(_load_fixture(), "SFO", "CDG", _CAPTURED_AT)
        # First result: Air France nonstop
        r = results[0]
        assert r.payload["airline"] == "Air France"
        assert r.payload["stops"] == 0
        assert r.payload["stops_text"] == "Nonstop"
        assert r.payload["price_text"] == "$849"
        assert r.payload["depart_time_text"] == "8:30 AM"
        assert r.payload["arrive_time_text"] == "6:45 AM"
    ...
    def test_prices_extracted(self) -> None:
        ...
        assert "$672" in prices
        assert "$715" in prices
        assert "$849" in prices
    ...
    def test_airlines_extracted(self) -> None:
        ...
        assert "Delta · Air France" in airlines
    ...
    def test_second_result_airline(self) -> None:
        results = parse_results_html(_load_fixture(), "SFO", "CDG", _CAPTURED_AT)
        assert results[1].payload["airline"] == "United · Lufthansa"
        assert results[1].payload["price_text"] == "$672"

    def test_third_result_airline(self) -> None:
        results = parse_results_html(_load_fixture(), "SFO", "CDG", _CAPTURED_AT)
        assert results[2].payload["airline"] == "Delta · Air France"
        assert results[2].payload["price_text"] == "$715"
